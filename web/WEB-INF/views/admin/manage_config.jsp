        ...
        <main class="flex-1 p-4 md:p-8 overflow-y-auto">
            ...
            <form method="POST" action="${pageContext.request.contextPath}/admin/config">
                ...
                <header class="flex justify-between items-center mb-8">
                    ...
                    <button type="submit" class="flex items-center gap-2 px-6 py-2.5 bg-[#00d4ff] text-black rounded-xl hover:bg-cyan-400 transition-colors font-bold shadow-[0_0_15px_rgba(0,212,255,0.3)]">
                    ...
                    <div class="glass-panel p-6 rounded-2xl border border-border-glass bg-bg-surface">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="MaxCapacity" value="${empty config['MaxCapacity'] ? '3' : config['MaxCapacity']}" min="1" max="10" class="w-full bg-black/40 border border-border-glass rounded-xl px-4 py-2 text-white focus:outline-none focus:border-[#00d4ff] transition-colors">
                            ...
                            <div>
                                ...
                                <select name="GracePeriod" class="w-full bg-black/40 border border-border-glass rounded-xl px-4 py-2 text-white focus:outline-none focus:border-[#00d4ff] transition-colors appearance-none">
                                    <option value="5" ${config['GracePeriod'] == '5' ? 'selected' : ''}>5 phút</option>
                                    <option value="10" ${config['GracePeriod'] == '10' ? 'selected' : ''}>10 phút</option>
                                    <option value="15" ${empty config['GracePeriod'] || config['GracePeriod'] == '15' ? 'selected' : ''}>15 phút</option>
                                    <option value="30" ${config['GracePeriod'] == '30' ? 'selected' : ''}>30 phút</option>
                                </select>
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <input type="number" name="PointsPerCurrencyUnit" value="${empty config['PointsPerCurrencyUnit'] ? '10' : config['PointsPerCurrencyUnit']}" class="w-24 bg-black/40 border border-border-glass rounded-xl px-4 py-2 text-center text-white focus:outline-none focus:border-[#00d4ff]">
                                ...
                                <div class="border-t border-border-glass pt-4">
                                    ...
                                    <div class="space-y-3">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <input type="number" name="SilverMultiplier" value="${empty config['SilverMultiplier'] ? '1.2' : config['SilverMultiplier']}" step="0.1" class="w-24 bg-black/40 border border-border-glass rounded-lg px-3 py-1.5 text-center text-white text-sm focus:outline-none focus:border-[#00d4ff]">
                                        </div>
                                        <div class="flex items-center justify-between">
                                            <span class="text-amber-400 text-sm font-medium">Gold</span>
                                            <input type="number" name="GoldMultiplier" value="${empty config['GoldMultiplier'] ? '1.5' : config['GoldMultiplier']}" step="0.1" class="w-24 bg-black/40 border border-amber-500/30 rounded-lg px-3 py-1.5 text-center text-white text-sm focus:outline-none focus:border-amber-400">
                                        </div>
                                        <div class="flex items-center justify-between">
                                            <span class="text-[#00d4ff] text-sm font-bold">Platinum</span>
                                            <input type="number" name="PlatinumMultiplier" value="${empty config['PlatinumMultiplier'] ? '2.0' : config['PlatinumMultiplier']}" step="0.1" class="w-24 bg-black/40 border border-[#00d4ff]/30 rounded-lg px-3 py-1.5 text-center text-white text-sm focus:outline-none focus:border-[#00d4ff]">
                                        </div>
                                    </div>
                                </div>
                            </div>
