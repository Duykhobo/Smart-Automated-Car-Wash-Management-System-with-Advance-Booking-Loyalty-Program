...
<html lang="vi">
    ...
    <style>
            ...
            input[name="services"]:checked + div h3 {
                color: #ffffff !important;
            }
            input[name="services"]:checked + div .radio-outer {
                border-color: #00d4ff !important;
            }
            input[name="services"]:checked + div .radio-inner {
                opacity: 1 !important;
            }
    ...
    <jsp:param name="activeMenu" value="history" />
                ...
                <form id="bookingForm" action="${pageContext.request.contextPath}/customer/booking_history" method="POST" class="space-y-8" onsubmit="return validateForm()" data-auto-validate="true">
                    ...
                    <section class="space-y-4">
                        ...
                        <div class="relative" id="vehicleSelectContainer">
                            <input type="hidden" name="vehicleId" id="vehicleIdInput" value="${bookingInfo.vehicleId}">
                                    ...
                                    <c:choose>
                                        <c:when test="${not empty vehicles}">
                                            <c:forEach var="v" items="${vehicles}">
                                                <c:set var="isDefaultVehicle" value="${v.licensePlate == bookingInfo.licensePlate}" />
                                                <c:choose>
                                                    <c:when test="${isDefaultVehicle}">
                                                        <div class="vehicle-option px-4 py-3 rounded-lg cursor-pointer hover:bg-white/10 transition-colors flex items-center justify-between group" data-value="${v.vehicleId}" data-size="${v.vehicleSize}" data-type="${v.vehicleTypeName}" data-default="true" data-text="${v.licensePlate}${not empty v.brand ? ' - '.concat(v.brand) : ''}">
                                                    ...
                                                    <c:otherwise>
                                                        <div class="vehicle-option px-4 py-3 rounded-lg cursor-pointer hover:bg-white/10 transition-colors flex items-center justify-between group" data-value="${v.vehicleId}" data-size="${v.vehicleSize}" data-type="${v.vehicleTypeName}" data-text="${v.licensePlate}${not empty v.brand ? ' - '.concat(v.brand) : ''}">
                                                            <span class="text-gray-300 group-hover:text-white font-medium transition-colors">${v.licensePlate} <c:if test="${not empty v.brand}"><span class="text-text-muted text-sm ml-1 font-normal">- ${v.brand}</span></c:if></span>
                                                            <i data-lucide="check" class="w-4 h-4 text-[#00d4ff] opacity-0 transition-opacity check-icon"></i>
                                                        </div>
                                                    </c:otherwise>
                                ...
                                <c:when test="${not empty services}">
                                    <c:forEach var="service" items="${services}" varStatus="status">
                                        <c:if test="${service.serviceType == 'Main' or empty service.serviceType}">
                                            <c:set var="isServiceChecked" value="${not empty bookingInfo.serviceNames and bookingInfo.serviceNames.contains(service.name)}" />
                                            <label class="block relative cursor-pointer group">
                                                <input type="radio" name="services" value="${service.serviceId}" data-base-price="${service.basePrice}" data-duration="${service.durationMinutes}" class="peer sr-only main-service" ${isServiceChecked ? 'checked' : ''}>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <c:forEach var="service" items="${services}">
                                <c:if test="${service.serviceType == 'Addon'}">
                                    <c:set var="isServiceChecked" value="${not empty bookingInfo.serviceNames and bookingInfo.serviceNames.contains(service.name)}" />
                                    <label class="block relative cursor-pointer group">
                                        <input type="checkbox" name="services" value="${service.serviceId}" data-base-price="${service.basePrice}" data-duration="${service.durationMinutes}" class="peer sr-only addon-service" ${isServiceChecked ? 'checked' : ''}>
                    ...
                    <!-- Promo Code -->
                    <section class="pt-6 border-t border-border-glass opacity-50 cursor-not-allowed">
                        <h2 class="font-display font-bold text-base text-white mb-3">Mã Khuyến Mãi / Voucher</h2>
                        <div class="flex items-center justify-between p-2 pl-4 glass-panel rounded-xl transition-all bg-black/20">
                            <input type="text" placeholder="Không hỗ trợ thay đổi Voucher khi sửa lịch hẹn" class="bg-transparent border-none outline-none text-sm w-full text-white placeholder:text-gray-500 min-w-0 cursor-not-allowed" disabled>
                            <button type="button" class="px-5 py-2.5 bg-gray-800 text-gray-500 rounded-lg font-bold text-sm whitespace-nowrap shrink-0 cursor-not-allowed" disabled>Bị khóa</button>
                        </div>
                    </section>
        ...
        <script>
            ...
            var servicePricesData = ${not empty servicePricesJson ? servicePricesJson : '{}'};
            // Server-injected vehicle size map: { vehicleId: vehicleSize }
            var vehicleSizeMap = {
                <c:forEach var="v" items="${vehicles}" varStatus="vs">
                    '${v.vehicleId}': '${not empty v.vehicleSize ? v.vehicleSize : "SEDAN"}'${not vs.last ? ',' : ''}
                </c:forEach>
            };
            // Default size for the booking's vehicle (from server)
            var initialVehicleSize = vehicleSizeMap['${bookingInfo.vehicleId}'] || 'SEDAN';
            window.currentGlobalDuration = 0;
            ...
            function showJSToast(type, message) {
                ...
                toast.className = `flex items-center w-full max-w-xs p-4 space-x-3 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 border-l-4 translate-x-full transition-transform duration-300 \${type === 'success' ? 'border-green-500' : 'border-red-500'}`;
                
                const iconBg = type === 'success' ? 'bg-green-100 dark:bg-green-800 text-green-500 dark:text-green-200' : 'bg-red-100 dark:bg-red-800 text-red-500 dark:text-red-200';
                        ...
                        .then(data => {
                            ...
                            data.forEach((slot, index) => {
                                ...
                                <label class="transition-transform group relative select-none \${disabled ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}" data-is-full="\${isFull}">
                                    <input type="radio" name="time" class="peer sr-only" value="\${slot.time}" \${disabled ? 'disabled' : ''} data-waitlist="\${isFull}" \${slot.time === oldTime ? 'checked' : ''}>
                            ...
                            // Initial call to set correct button state if oldTime slot is checked
                            updateSubmitButton();
            ...
            function updateSubmitButton() {
                ...
                if (selectedTime && selectedTime.getAttribute('data-waitlist') === 'true') {
                    submitBtn.innerHTML = `<i data-lucide="clock" class="w-4 h-4 mr-2"></i> CHUYỂN VÀO DANH SÁCH CHỜ`;
                    ...
                    submitBtn.innerHTML = 'CẬP NHẬT LỊCH HẸN';
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                function updatePrice() {
                    // Default to server-injected vehicle size; update when user changes vehicle
                    let currentVehicleSize = initialVehicleSize;
                    const vehicleId = document.getElementById('vehicleIdInput').value;
                    
                    if (vehicleId) {
                        // 1st: try server-injected map (most reliable)
                        if (vehicleSizeMap && vehicleSizeMap[vehicleId]) {
                            currentVehicleSize = vehicleSizeMap[vehicleId];
                        }
                        // 2nd: try DOM data-size attribute (for dynamically selected vehicles)
                        const selectedVehicleOption = document.querySelector(`.vehicle-option[data-value="\${vehicleId}"]`);
                        if (selectedVehicleOption) {
                            const domSize = (selectedVehicleOption.getAttribute('data-size') || '').trim();
                            if (domSize) {
                                currentVehicleSize = domSize;
                            } else {
                    ...
                    // Single loop: update display price AND accumulate total in one pass
                    serviceCheckboxes.forEach(checkbox => {
                        const sid = checkbox.value;
                        const basePrice = parseInt(checkbox.getAttribute('data-base-price')) || 0;
                    ...
                    if (priceDisplay) {
                        priceDisplay.innerHTML = new Intl.NumberFormat('vi-VN').format(finalPrice) + ' <span class="text-sm text-text-muted font-sans font-normal">đ</span>';
                    }
                ...
                if (trigger && dropdown) {
                    ...
                    options.forEach(option => {
                        ...
                        // Auto-select: highlight the vehicle that matches the booking's vehicleId
                    ...
                    // If no vehicle was pre-selected, pick the first one
            ...
            function validateForm() {
                const selectedServices = document.querySelectorAll('input[name="services"]:checked');
                if (selectedServices.length === 0) {
                    showJSToast('error', "Vui lòng chọn ít nhất một dịch vụ!");
                    return false;
                }
