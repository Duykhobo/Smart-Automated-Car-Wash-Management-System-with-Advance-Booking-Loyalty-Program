...
<body class="bg-bg-primary text-text-primary antialiased overflow-x-hidden selection:bg-[#00d4ff] selection:text-black flex">
                                ...
                                <td class="flex lg:table-cell justify-between items-center px-2 lg:px-6 py-4 lg:text-right mt-2 lg:mt-0 border-t lg:border-none border-border-glass">
                                    ...
                                    <div class="flex items-center justify-end gap-2">
                                        <button class="w-8 h-8 rounded-lg text-text-muted hover:text-white flex items-center justify-center transition-colors border border-transparent hover:border-border-glass bg-white/5 hover:bg-white/10" 
                                                onclick="openEditModal(${service.serviceId}, '${service.name.replace("'", "\\'")}', ${service.basePrice}, ${service.durationMinutes}, '${service.isIsActive()}', ${service.priceSedan}, ${service.priceSuv}, ${service.priceXlarge})">
            ...
            <form action="${pageContext.request.contextPath}/admin/services" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" class="w-full bg-bg-primary border border-border-glass rounded-xl px-4 py-2.5 text-white focus:border-[#00d4ff] outline-none transition-all" placeholder="Ví dụ: Rửa Wax bóng cao cấp" required>
                    </div>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm text-text-muted mb-1 font-medium">Giá Cơ Bản (VND)</label>
                            <input type="number" name="basePrice" class="w-full bg-bg-primary border border-border-glass rounded-xl px-4 py-2.5 text-white focus:border-[#00d4ff] outline-none transition-all" placeholder="150000" required>
                        </div>
                        <div>
                            <label class="block text-sm text-text-muted mb-1 font-medium">Thời lượng (Phút)</label>
                            <input type="number" name="duration" class="w-full bg-bg-primary border border-border-glass rounded-xl px-4 py-2.5 text-white focus:border-[#00d4ff] outline-none transition-all" placeholder="45" required>
                        </div>
                    </div>
                    
                    <div class="p-4 rounded-xl bg-white/5 border border-border-glass space-y-3">
                        <p class="text-xs text-[#00d4ff] font-bold uppercase tracking-wider">Thiết lập bảng giá chi tiết cỡ xe</p>
                        <div class="grid grid-cols-3 gap-2">
                            <div>
                                <label class="block text-xs text-slate-400 mb-1">SEDAN (VND)</label>
                                <input type="number" name="priceSedan" class="w-full bg-bg-primary border border-border-glass rounded-lg px-2.5 py-2 text-sm text-white focus:border-[#00d4ff] outline-none transition-all" required>
                            </div>
                            <div>
                                <label class="block text-xs text-slate-400 mb-1">SUV (VND)</label>
                                <input type="number" name="priceSuv" class="w-full bg-bg-primary border border-border-glass rounded-lg px-2.5 py-2 text-sm text-white focus:border-[#00d4ff] outline-none transition-all" required>
                            </div>
                            <div>
                                <label class="block text-xs text-slate-400 mb-1">XLARGE (VND)</label>
                                <input type="number" name="priceXlarge" class="w-full bg-bg-primary border border-border-glass rounded-lg px-2.5 py-2 text-sm text-white focus:border-[#00d4ff] outline-none transition-all" required>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="glass-panel w-full max-w-lg bg-bg-surface border border-border-glass rounded-2xl p-6 relative shadow-[0_0_50px_rgba(0,0,0,0.8)]">
            ...
            <form action="${pageContext.request.contextPath}/admin/services" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm text-text-muted mb-1 font-medium">Tên Dịch Vụ</label>
                        <input type="text" id="editServiceName" name="name" class="w-full bg-bg-primary border border-border-glass rounded-xl px-4 py-2.5 text-white focus:border-[#00d4ff] outline-none transition-all" required>
                    </div>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm text-text-muted mb-1 font-medium">Giá Cơ Bản (VND)</label>
                            <input type="number" id="editBasePrice" name="basePrice" class="w-full bg-bg-primary border border-border-glass rounded-xl px-4 py-2.5 text-white focus:border-[#00d4ff] outline-none transition-all" required>
                        </div>
                        <div>
                            <label class="block text-sm text-text-muted mb-1 font-medium">Thời gian (Phút)</label>
                            <input type="number" id="editDuration" name="duration" class="w-full bg-bg-primary border border-border-glass rounded-xl px-4 py-2.5 text-white focus:border-[#00d4ff] outline-none transition-all" required>
                        </div>
                    </div>
                    
                    <div class="p-4 rounded-xl bg-white/5 border border-border-glass space-y-3">
                        <p class="text-xs text-[#00d4ff] font-bold uppercase tracking-wider">Cập nhật giá theo kích cỡ xe</p>
                        <div class="grid grid-cols-3 gap-2">
                            <div>
                                <label class="block text-xs text-slate-400 mb-1">SEDAN (VND)</label>
                                <input type="number" id="editPriceSedan" name="priceSedan" class="w-full bg-bg-primary border border-border-glass rounded-lg px-2.5 py-2 text-sm text-white focus:border-[#00d4ff] outline-none transition-all" required>
                            </div>
                            <div>
                                <label class="block text-xs text-slate-400 mb-1">SUV (VND)</label>
                                <input type="number" id="editPriceSuv" name="priceSuv" class="w-full bg-bg-primary border border-border-glass rounded-lg px-2.5 py-2 text-sm text-white focus:border-[#00d4ff] outline-none transition-all" required>
                            </div>
                            <div>
                                <label class="block text-xs text-slate-400 mb-1">XLARGE (VND)</label>
                                <input type="number" id="editPriceXlarge" name="priceXlarge" class="w-full bg-bg-primary border border-border-glass rounded-lg px-2.5 py-2 text-sm text-white focus:border-[#00d4ff] outline-none transition-all" required>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function openEditModal(id, name, basePrice, duration, isActive, priceSedan, priceSuv, priceXlarge) {
            ...
            document.getElementById('editPriceSedan').value = priceSedan;
            document.getElementById('editPriceSuv').value = priceSuv;
            document.getElementById('editPriceXlarge').value = priceXlarge;
