...
## 3.A. MÔ HÌNH HÓA HỆ THỐNG (UML DIAGRAMS)

### 3.A.1. Sơ đồ Use Case (Use Case Diagram)
...
### 3.A.2. Sơ đồ Thực thể Liên kết (Entity Relationship Diagram - ERD)
...
erDiagram
	...
	WASH_RECORD {
		...
		datetime ActualEnd  ""  
		decimal LPRConfidence  ""  
		nvarchar OperatorNotes  ""  
	}
	...
	CUSTOMER ||--o{ POINT_LEDGER : "earns/spends"
	CUSTOMER ||--o{ VOUCHER : "owns"
	SERVICE ||--o{ BOOKING : "includes"
	BOOKING_SLOT_CAPACITY ||--o{ BOOKING : "allocates"
	BOOKING ||--o| WASH_RECORD : "results in"
	BOOKING ||--o| VOUCHER : "uses"
	CUSTOMER ||--o{ PROMOTION : "eligible for"
```

### 3.A.3. Sơ đồ Máy trạng thái (State Machine Diagram - Loyalty Tiers)
...
## 3.B. ĐẶC TẢ USE CASE CHI TIẾT (DETAILED USE CASE)
...
### UC-08: Xác nhận hoàn thành dịch vụ (Check-out & Trigger Points)
| Thành phần | Đặc tả |
| :--- | :--- |
| **Actor** | Admin / Staff |
| **Mô tả** | Nhân viên xác nhận hoàn thành dịch vụ cho một Booking để hệ thống ghi nhận doanh thu và kích hoạt cộng điểm. |
| **Pre-condition** | Nhân viên đã đăng nhập và xe đang ở trạng thái `InProgress`. |
| **Main Flow** | 1. Nhân viên xem danh sách xe đang rửa.<br>2. Chọn xe đã xong và nhấn "Hoàn thành".<br>3. Hệ thống update bảng `Bookings` thành `Completed`.<br>4. Gọi hàm Trigger Loyalty Engine để cộng điểm và xét thăng hạng.<br>5. Ghi nhận thời gian `ActualEndTime` vào bảng `WashRecords`. |
| **Alternative Flow** | (3a) Nếu mạng lỗi, hệ thống retry hoặc cho phép nhân viên thao tác lại. |
| **Post-condition** | Booking hoàn tất, điểm Loyalty được cộng vào tài khoản khách hàng. |

### UC-09: Hủy lịch & Xử lý No-Show (Cancellation Flow)
| Thành phần | Đặc tả |
| :--- | :--- |
| **Actor** | Customer / Admin |
| **Mô tả** | Khách hàng tự hủy lịch hẹn trước giờ quy định, hoặc Admin đánh dấu xe không đến (No-Show). |
| **Pre-condition** | Có Booking đang ở trạng thái `Pending` hoặc `Confirmed`. |
| **Main Flow (Customer)** | 1. Khách vào chi tiết lịch -> Bấm Hủy.<br>2. Hệ thống check thời gian (có trước 12 tiếng không).<br>3. Nếu hợp lệ: Đổi status thành `Cancelled`, trả lại Voucher thành `Unused`, giảm `CurrentBooked` trong slot đi 1. |
| **Main Flow (Admin)** | 1. Hết ngày, nhân viên check danh sách.<br>2. Thấy xe không đến -> Bấm "No-Show".<br>3. Hệ thống đổi status thành `NoShow`, đánh dấu vi phạm, thu hồi Voucher. |
| **Alternative Flow** | (Customer - 2a) Nếu hủy muộn (dưới 12 tiếng), cảnh báo sẽ không được hoàn Voucher. |
| **Post-condition** | Slot trống được giải phóng (nếu hủy hợp lệ). Cập nhật vi phạm (nếu No-show). |

### UC-10: Cập nhật thông tin phương tiện
| Thành phần | Đặc tả |
| :--- | :--- |
| **Actor** | Customer |
| **Mô tả** | Khách hàng cập nhật biển số xe mới khi đổi xe để hệ thống LPR quét chính xác. |
| **Pre-condition** | Khách hàng đã Login. |
| **Main Flow** | 1. Khách vào Profile -> Chọn Sửa biển số.<br>2. Nhập biển mới.<br>3. Hệ thống check trùng lặp trong DB.<br>4. Cập nhật thành công. |
| **Alternative Flow** | (3a) Nếu biển số đã tồn tại trên tài khoản khác -> Báo lỗi. |
| **Post-condition** | Biển số xe được cập nhật, sử dụng cho lần check-in tiếp theo. |

---

---

## 4. PHÂN TÍCH QUY TẮC NGHIỆP VỤ NÂNG CAO
(BUSINESS RULES)
...
*   **Hạ hạng (Xét duyệt định kỳ hàng tháng - Monthly Review):**
    ...
    *   *Giải pháp Kỹ thuật (System Architecture):* Để thực hiện tác vụ chạy ngầm định kỳ này một cách chặt chẽ, hệ thống sẽ sử dụng công cụ **SQL Server Agent** để gọi tự động một **Stored Procedure** quét toàn bộ data hạ hạng (tối ưu nhất về mặt hiệu suất), hoặc viết một Background Thread sử dụng `ScheduledExecutorService` khởi chạy cùng Tomcat Servlet.
...
*   *Các trường:*
    ...
    *   `FullName` (NVARCHAR(100), Not Null): Họ và tên đầy đủ của khách hàng.
...
### 6.4 Bảng: `BookingSlotCapacity`
*   *Mục đích:* Bảng phụ kiểm soát sức chứa khoang rửa (Concurrency Control). Đảm bảo không bị Over-booking bằng cơ chế khóa dòng (Rowlock).
*   *Các trường:*
    *   `SlotID` (INT, Primary Key, Identity).
    *   `SlotDate` (DATE, Not Null).
    *   `TimeSlot` (TIME, Not Null).
    *   `MaxCapacity` (INT, Default 3): Số lượng tối đa xe trong 1 slot.
    *   `CurrentBooked` (INT, Default 0): Số Booking hiện tại đã xác nhận.
    *   *Constraint:* `UNIQUE(SlotDate, TimeSlot)`

### 6.8 Bảng: `WashRecords`
...
8.  **UC-08: Xác nhận hoàn thành dịch vụ** (Tác nhân: Admin / Staff)
9.  **UC-09: Hủy lịch & Xử lý No-Show** (Tác nhân: Customer / Admin)
10. **UC-10: Cập nhật thông tin phương tiện** (Tác nhân: Customer)
...
3.  **Luồng xử lý chính:**
    ...
    *   `BookingServlet` chặn yêu cầu và mở một SQL Transaction (gọi lệnh `connection.setAutoCommit(false)`) để kiểm tra tính toàn vẹn dữ liệu.
        > [!CAUTION]
        > **Bảo vệ kết nối DB:** Quá trình mở Transaction này bắt buộc phải bọc trong khối `try-catch-finally`, và phải có lệnh `connection.close()` ở block `finally`. Điều này đảm bảo dù có Exception văng ra giữa chừng, Connection vẫn được trả về Pool an toàn.
...
*   `POST /api/loyalty/redeem`
    ...
    *   *Response:*
        ...
        ```
*   `PUT /api/customer/vehicle`
    *   *Payload:* `{"newLicensePlate": "51K-999.99"}`
    *   *Response:* `{"status": "success", "message": "Cập nhật biển số thành công"}`
...
*   `GET /api/booking/slots`
    *   *Query Parameters:* `?date=2026-05-20`
    *   *Response:* `[{"time": "09:00:00", "available": 3}, {"time": "10:00:00", "available": 1}]`
*   `PUT /api/booking/{id}/cancel`
    *   *Payload:* `{}`
    *   *Response:* `{"status": "success", "message": "Hủy lịch thành công, Voucher đã được hoàn lại"}`
...
### 8.5 Nhóm API Quản lý & Vận hành (Admin/Staff)
*   `PUT /api/admin/booking/{id}/complete`
    *   *Payload:* `{}`
    *   *Response:* `{"status": "success", "message": "Đã hoàn thành dịch vụ, khách hàng được cộng 240 điểm"}`

---

## 9.A. MOCKUPS - THIẾT KẾ GIAO DIỆN (UI WIREFRAMES / MOCKUPS)
...
*   *Rủi ro tiềm ẩn:* Vào các khung giờ cao điểm, hàng chục khách hàng cùng lúc nhấp nút "Xác nhận đặt lịch" cho slot trống cuối cùng. Nếu đếm số lượng Booking trực tiếp trên bảng BOOKING có thể dính lỗi Phantom Read, dẫn đến Over-booking (đặt lố slot).
*   *Phương án giải quyết (Thiết kế SA):* Sử dụng bảng phụ `BookingSlotCapacity` và khóa dòng vật lý với UPDLOCK.
    ```sql
    -- Bước 1: Khóa dòng Slot
    SELECT CurrentBooked, MaxCapacity 
    FROM BOOKING_SLOT_CAPACITY WITH (UPDLOCK, ROWLOCK) 
    WHERE SlotDate = ? AND TimeSlot = ?
    
    -- Bước 2: Kiểm tra CurrentBooked < MaxCapacity, sau đó Update
    UPDATE BOOKING_SLOT_CAPACITY SET CurrentBooked = CurrentBooked + 1 
    WHERE SlotDate = ? AND TimeSlot = ?
    ```
    Truy vấn này ép luồng truy cập sau phải tạm dừng ở tầng database cho đến khi luồng đầu tiên hoàn tất, ngăn chặn tuyệt đối tình trạng Over-booking và tránh Deadlock hiệu quả.
